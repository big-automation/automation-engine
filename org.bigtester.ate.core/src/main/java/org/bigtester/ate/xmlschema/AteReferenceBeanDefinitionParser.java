 ...
 *
 */
public class AteReferenceBeanDefinitionParser extends AbstractBeanDefinitionParser {
	...
	protected AbstractBeanDefinition parseInternal(@Nullable Element element,
		...
		String ateDefRef = element
			.getAttribute("ateXmlElementID");
	if (StringUtils.isEmpty(ateDefRef)) {
		throw GlobalUtils
				.createNotInitializedException("ateXmlElementID value");
	} else {
		AbstractBeanDefinition retVal = (AbstractBeanDefinition) parserContext.getRegistry()
				.getBeanDefinition(ateDefRef);
