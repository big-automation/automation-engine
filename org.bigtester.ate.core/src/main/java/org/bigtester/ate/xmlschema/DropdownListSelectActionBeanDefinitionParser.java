 ...
 *
 */
public class DropdownListSelectActionBeanDefinitionParser extends AbstractBeanDefinitionParser {
	...
	protected @Nullable AbstractBeanDefinition parseInternal(@Nullable Element element,
		...
		// Here we parse the Spring elements such as < property>
				...
				if (StringUtils.hasText(selections)) {
					bDef.setAttribute(XsdElementConstants.ATTR_DROPDOWNLISTSELECTACTION_SELECTIONS, selections);
					bDef.getConstructorArgumentValues().addGenericArgumentValue(selections);
				} else {
					bDef.setAttribute(XsdElementConstants.ATTR_DROPDOWNLISTSELECTACTION_SELECTIONS, "");
				}
