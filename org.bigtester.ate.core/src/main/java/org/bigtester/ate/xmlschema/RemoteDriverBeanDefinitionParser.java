...
public class RemoteDriverBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	...
	protected void doParse(@Nullable Element element,
		...
		String platform = element
				.getAttribute(XsdElementConstants.ATTR_REMOTEDRIVER_PLATFORM);
		if (!StringUtils.isEmpty(platform)) {
			bean.addConstructorArgValue(platform);
		}
		String url = element
				.getAttribute(XsdElementConstants.ATTR_REMOTEDRIVER_URL);
		if (!StringUtils.isEmpty(url)) {
			bean.addConstructorArgValue(url);
		}
