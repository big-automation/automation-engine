...
public class UploadFileActionBeanDefinitionParser extends AbstractBeanDefinitionParser {
	...
	protected @Nullable AbstractBeanDefinition parseInternal(@Nullable Element element,
		...
		// Here we parse the Spring elements such as < property>
				...
				String data = element
						.getAttribute(XsdElementConstants.ATTR_UPLOADFILEACTION_DATAVALUE);
...
//					parserContext.getRegistry().registerBeanDefinition(element.getAttribute("id") + "_ELEMENTINPUTDATA_ID", inputDataValueDef);
					bDef.setAttribute(XsdElementConstants.ATTR_UPLOADFILEACTION_DATAVALUE, data);
					bDef.getConstructorArgumentValues().addGenericArgumentValue( new RuntimeBeanReference(data));
				} else {
					bDef.setAttribute(XsdElementConstants.ATTR_UPLOADFILEACTION_DATAVALUE, "");
				}
